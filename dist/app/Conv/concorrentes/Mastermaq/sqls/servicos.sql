SELECT
BPM_DADOSTOMADOR.IDOWNER,
BPM_DADOSTOMADOR.IDDADOSTOMADOR,
(SELECT BPM_PESSOA.NOMEPESSOA FROM NG.DBO.BPM_PESSOA WHERE BPM_PESSOA.IDPESSOA = BPM_DADOSTOMADOR.IDPESSOA) AS NOME,
BPM_DADOSPESSOAJURIDICA.CNPJ,
(SELECT BPM_PESSOA.NUMEROCEI FROM NG.DBO.BPM_PESSOA WHERE BPM_PESSOA.IDPESSOA = BPM_DADOSTOMADOR.IDPESSOA) AS NUMEROCEI,
  ...
  WHERE CONT1.IDTIPOCONTATO = 3
	...
	AND CONT1.DATAHORALOG = (SELECT MAX(CONT2.DATAHORALOG)
                              ...
                              WHERE CONT2.IDTIPOCONTATO = 1
                                AND CONT2.IDPESSOA = BPM_DADOSPESSOAJURIDICA.IDPESSOA)) AS EMAIL,
(SELECT CODIGOGPS FROM NG_DOMINIO.DBO.DOM_CODIGOGPS WHERE DOM_CODIGOGPS.IDCODIGOGPS = BPM_DADOSTOMADOR.IDGPS) AS GPS,
(SELECT CODIGOSEFIPRECOLHIMENTO FROM NG_DOMINIO.DBO.DOM_SEFIPRECOLHIMENTO WHERE DOM_SEFIPRECOLHIMENTO.IDSEFIPRECOLHIMENTO = BPM_DADOSTOMADOR.IDSEFIPRECOLHIMENTO) AS SEFIPRECOLHIMENTO,
(SELECT CODIGOFPAS FROM NG_DOMINIO.DBO.DOM_FPAS WHERE DOM_FPAS.IDFPAS = BPM_DADOSTOMADOR.IDFPAS) AS FPAS,
(SELECT CODIGOTERCEIRO FROM NG_DOMINIO.DBO.DOM_CODIGOTERCEIRO WHERE DOM_CODIGOTERCEIRO.IDCODIGOTERCEIRO = BPM_DADOSTOMADOR.IDTERCEIRO) AS TERCEIRO,
BPM_DADOSTOMADOR.PERCENTUALRAT,
BPM_DADOSTOMADOR.PERCENTUALTERCEIRO,
(SELECT BPM_PESSOA.CAEPF FROM NG.DBO.BPM_PESSOA WHERE BPM_PESSOA.IDPESSOA = BPM_DADOSTOMADOR.IDPESSOA) AS CAEPF,
(SELECT BPM_PESSOA.IDTIPOCAEPF FROM NG.DBO.BPM_PESSOA WHERE BPM_PESSOA.IDPESSOA = BPM_DADOSTOMADOR.IDPESSOA) AS IDTIPOCAEPF

FROM NG.DBO.BPM_DADOSTOMADOR

LEFT OUTER JOIN NG.DBO.BPM_PESSOAENDERECO
ON BPM_DADOSTOMADOR.IDPESSOA = BPM_PESSOAENDERECO.IDPESSOA,

NG.DBO.BPM_DADOSPESSOAJURIDICA

WHERE BPM_DADOSTOMADOR.IDPESSOA = BPM_DADOSPESSOAJURIDICA.IDPESSOA
