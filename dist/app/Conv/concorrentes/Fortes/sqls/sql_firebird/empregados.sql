SELECT EPG.EMP_CODIGO,
	...
	EPG.TPINSCANT,
	EPG.NUMEROALVARA,
	(SELECT FIRST 1 INSCRICAO
	 FROM EEA
	 WHERE CODIGO = (
		 SELECT FIRST 1 EEA_CODIGO_EFETIVACAO
		 FROM SEP
	...
	(SELECT FIRST 1 INSCRICAO
	 FROM EEA
	 WHERE CODIGO = (
		 SELECT FIRST 1 EEA_CODIGO_PRATICA
		 FROM SEP
		 WHERE EPG.EMP_CODIGO = SEP.EMP_CODIGO
		   AND EPG.CODIGO = SEP.EPG_CODIGO
		 ORDER BY SEP.DATA DESC
	 )) AS INSCRICAO_PRATICA,
	(SELECT FIRST 1 INDCONTRATAAPRENDIZ
	 FROM SEP
	 WHERE EPG.EMP_CODIGO = SEP.EMP_CODIGO
	   AND EPG.CODIGO = SEP.EPG_CODIGO
	 ORDER BY SEP.DATA DESC),
	...
	SEP_Antigo.EXPAGENOCIV AS OCORRENCIA_SEFIP,
