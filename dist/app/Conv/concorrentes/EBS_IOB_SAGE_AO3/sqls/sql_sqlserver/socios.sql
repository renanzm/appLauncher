SELECT 
    ...
    COALESCE((SELECT MAX(MUNICIPIOG.CD_MUNICIPIO_CONCLA )
              ...
              WHERE MUNICIPIOG.CD_MUNICIPIO = DIRETOR.CD_MUNICIPIO  
                AND MUNICIPIOG.ESTADO = DIRETOR.ESTADO), 
             (SELECT MAX(MUNICIPIOCGW.CD_MUNICIPIO_IBGE )
    ...
    COALESCE((SELECT MAX(MUNICIPIOG.CD_MUNICIPIO_CONCLA )
              ...
              WHERE MUNICIPIOG.CD_MUNICIPIO = DIRETOR.CD_MUNICIPIO_NASCIMENTO  
                AND MUNICIPIOG.ESTADO = DIRETOR.ESTADO_NASCIMENTO), 
             (SELECT MAX(MUNICIPIOCGW.CD_MUNICIPIO_IBGE )
              FROM EBS_Cordilheira.DBO.MUNICIPIOCGW 
...
INNER JOIN CRDEmpresaQuadroSocio ON DIRETOR.CD_EMPRESA = CRDEmpresaQuadroSocio.CD_EMPRESA
    AND DIRETOR.CD_DIRETOR = CRDEmpresaQuadroSocio.CD_SOCIO
LEFT JOIN (
    SELECT
        CD_EMPRESA,
        CD_DIRETOR,
...
) SUBQ ON DIRETOR.CD_EMPRESA = SUBQ.CD_EMPRESA 
    AND DIRETOR.CD_DIRETOR = SUBQ.CD_DIRETOR
