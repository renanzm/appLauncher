...
/* SELECIONA O PROXIMO VENCIMENTO DE FERIAS */
...
/*(SELECT MAX(FUNTOMADOR.CD_TOMADOR)
  ...
  COALESCE(CRHFUNTRANSFERENCIAEMPRESAS.MES_ANO_TRANSFERENCIA,CRHFUNCIONARIOTIPOADMISSAO.DT_INICIO_VINCULO) AS DATA_TRANSFERENCIA,
  ...
  (SELECT MAX(S.TIPO_SALARIO) 
     FROM FUNSALARIO AS S
	WHERE S.CD_EMPRESA= FUNFUNCIONAL.CD_EMPRESA
	  AND S.CD_FUNCIONARIO= FUNFUNCIONAL.CD_FUNCIONARIO
	  AND S.DT_SALARIO = (SELECT MAX(S2.DT_SALARIO) 
	                          FROM FUNSALARIO AS S2
...
LEFT OUTER JOIN CRHFUNCIONARIOTIPOADMISSAO
...
LEFT OUTER JOIN CRHFUNCIONARIOREINTEGRADO
